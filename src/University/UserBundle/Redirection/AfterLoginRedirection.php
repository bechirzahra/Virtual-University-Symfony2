...
namespace University\UserBundle\Redirection;
...
class AfterLoginRedirection implements AuthenticationSuccessHandlerInterface
{
    ...
    public function onAuthenticationSuccess(Request $request, TokenInterface $token)
    {
        ...
        elseif (in_array('ROLE_ADMIN', $rolesTab, true) )
            $redirection = new RedirectResponse($this->router->generate('homeAd'));
        // sinon, s'il s'agit d'un commercial on le redirige vers le CRM
        elseif (in_array('ROLE_ENSEIGNANT', $rolesTab, true))
            $redirection = new RedirectResponse($this->router->generate('homeEns'));
        elseif (in_array('ROLE_CHEFENTREPRISE', $rolesTab, true))
            $redirection = new RedirectResponse($this->router->generate('homeCh'));
        // sinon il s'agit d'un membre
        else
            $redirection = new RedirectResponse($this->router->generate('homeEt'));
